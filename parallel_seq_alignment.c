...
//Cuda functions:
extern int computeOnGPU(Seq_Info *seq_array, int arr_size,char* seq1,int seq1_length, int* score_table, int table_size);
...
void master_Work(int reminder)
{
    ...
    worker_seq_arr = (Seq_Info *)malloc(reminder * sizeof(Seq_Info));
    ...
    worker_seq_arr[i] = master_seq_arr[init_pack.num_of_seqs - reminder + i];
    }
//CUDA~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  
    int dummy;
    dummy = computeOnGPU(worker_seq_arr, reminder, init_pack.seq1, init_pack.seq1_len, init_pack.score_table, TABLE_SIZE);

    printf("[0] ~~> finished CUDA part returned (%d)\n",dummy);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    for (int i = 0; i < reminder; i++)
    {
        ...
        master_seq_arr[init_pack.num_of_seqs-reminder+i] = worker_seq_arr[i];
    }
    free(worker_seq_arr);
}
